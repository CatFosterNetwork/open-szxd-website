...
<template>
  ...
  >
    <div class="ms-3 text-xl" v-if="date != 'N/A'">
      Automatic check-in has been <span class="text-green-500 dark:text-green-400">activated until {{ date }}</span>.
      Please log in again to refresh after expiration.
    </div>
    <div class="ms-3 text-xl" v-else>
      Automatic check-in has been <span class="text-red-500 dark:text-red-400">deactivated</span>.
    </div>
  </div>
</template>

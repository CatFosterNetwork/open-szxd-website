...
const onSubmit = () => {
  ...
  if (user.value.balance < balance.value) {
    toast.add({ title: "余额不足", color: "error" });
    return;
  }
  if (
    !selected1.value ||
...
<template #body>
  <div class="flex flex-col p-5">
    ...
    >
      <USelectMenu v-model="selected1" :items="types" />
      <USelectMenu v-model="selected2" :items="valid_time" />
    </view>
    ...
    <UModal v-model="submitModal">
      <div class="p-4">
        ...
        <div class="flex justify-end mt-4">
          <UButton color="neutral" variant="subtle" @click="submitModal = false">取消</UButton>
    ...
    <view class="grow my-4">
      ...
      >
        ...
        >
          <view>{{ key }}</view>
          <UButton color="neutral" variant="subtle" size="sm" @click="copyKey(key)" class="ml-5"
            >复制</UButton
          >
        </view>
      </UPageCard>
